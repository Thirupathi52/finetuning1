# -*- coding: utf-8 -*-
"""tool_calling_finetune.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1NQaTpZauawEWXIepPJyu01ZdQrKnmyuX
...
| Cancel Order | `cancel_order` | "I would like to cancel my order" |
...
SYSTEM_PROMPT = """You are a customer support assistant. You have access to exactly three tools:
...
3. cancel_order: Use when the customer asks for cancel order, not happy with order/item, disappointed with item.
   Parameters: {"order_id": string}
...
TEST_QUERIES = [
    ...
    # Ambiguous "order" but intent is termination -> cancel_order
    "I need to pull the plug on my most recent transaction before it leaves the warehouse.",
    ...
    # Direct but informal -> cancel_order
    "can u guys kill order ORD-1102? i dont need it anymore",
    # Multi-intent, primary is tracking → track_order
    "Where the hell is ORD-5523? Also, do you have any human I can connect to?",
    # Mistake in purchase -> cancel_order
    "Wait, I just realized I clicked the wrong address for my latest purchase. Stop the shipment!",
    # Says "order", disappointed with order -> cancel_order
    "I am really disappointed with my order",
    # Specific reason (financial/accidental) -> cancel_order
    "My kid grabbed my phone and placed an order by mistake. How do I nix this?",
    ...
    # I would like to cancel my order -> cancel_order
    "I have changed my mind on the order",
    # Says "order", dislike the product on order -> cancel_order
    "My friend disliked the product on the order",
    # Frustrated with delay, wants to back out -> cancel_order
    "It’s taking way too long to ship ORD-9921. Just forget it and give me my money back.",
    ...
    # Found it cheaper elsewhere -> cancel_order
    "I found the same headphones for half the price on another site, so I don't want the ones from ORD-3342 anymore.",
    ...
    # Vague dissatisfaction leading to cancellation -> cancel_order
    "I've had a change of heart regarding my purchase from this morning. Please don't send it.",
]
...
#------------------------------------------
cancel_order_examples = [
    ("I want to cancel my order ORD-1001.", {"order_id": "ORD-1001"}),
    ("Can you stop the shipment for ORD-2233? I changed my mind.", {"order_id": "ORD-2233"}),
    ("Please cancel ORD-5567 immediately.", {"order_id": "ORD-5567"}),
    ("I accidentally ordered twice. Cancel ORD-8899 please.", {"order_id": "ORD-8899"}),
    ("Is it possible to void order ORD-3344?", {"order_id": "ORD-3344"}),
    ("I need to terminate my order ORD-7712.", {"order_id": "ORD-7712"}),
    ("Delete ORD-4421 from my account. I don't want it anymore.", {"order_id": "ORD-4421"}),
    ("Stop processing ORD-6650, I found a better price elsewhere.", {"order_id": "ORD-6650"}),
    ("Can I get a refund and cancel ORD-1199?", {"order_id": "ORD-1199"}),
    ("I'm trying to cancel ORD-8801 but the button isn't working.", {"order_id": "ORD-8801"}),
    ("ORD-3009 was a mistake. Please cancel it.", {"order_id": "ORD-3009"}),
    ("My son used my card for ORD-2277. Please cancel and refund.", {"order_id": "ORD-2277"}),
    ("ORD-5543 — I want to revoke this order.", {"order_id": "ORD-5543"}),
    ("If ORD-9902 hasn't shipped yet, please cancel it.", {"order_id": "ORD-9902"}),
    ("The delivery is too late. Cancel ORD-1123.", {"order_id": "ORD-1123"}),
    ("I need to kill order ORD-6677.", {"order_id": "ORD-6677"}),
    ("Please remove ORD-4489 from my active orders.", {"order_id": "ORD-4489"}),
    ("I've been waiting too long for ORD-3388. Just cancel it.", {"order_id": "ORD-3388"}),
    ("I bought the wrong item in ORD-7766. Cancel please.", {"order_id": "ORD-7766"}),
    ("ORD-5512 is no longer needed. Cancel it.", {"order_id": "ORD-5512"}),
    ("Can you help me cancel ORD-2201 before it ships out?", {"order_id": "ORD-2201"}),
    ("Hey, I need to back out of ORD-8834.", {"order_id": "ORD-8834"}),
    ("Cancel ORD-1155 and send a confirmation email.", {"order_id": "ORD-1155"}),
    ("I'm moving to a new address, so cancel ORD-9943.", {"order_id": "ORD-9943"}),
    ("Cancellation request for ORD-6230.", {"order_id": "ORD-6230"}),
    ("I want to opt out of ORD-7188.", {"order_id": "ORD-7188"}),
    ("My wife said we don't need ORD-4455. Cancel it.", {"order_id": "ORD-4455"}),
    ("Disregard ORD-3667, I don't want it.", {"order_id": "ORD-3667"}),
    ("ORD-8102. Cancel. Now.", {"order_id": "ORD-8102"}),
    ("Please process a cancellation for ORD-5079.", {"order_id": "ORD-5079"}),
    ("I'd like to void ORD-2918 please.", {"order_id": "ORD-2918"}),
    ("Is there any way to stop ORD-4600 from being sent?", {"order_id": "ORD-4600"}),
    ("ORD-7331 has been processing too long. Cancel it.", {"order_id": "ORD-7331"}),
    ("I'm requesting a cancellation for ORD-1844.", {"order_id": "ORD-1844"}),
    ("Because of the delay, I want to cancel ORD-9215.", {"order_id": "ORD-9215"}),
    ("Forget about ORD-6503, I'm going to a local store.", {"order_id": "ORD-6503"}),
    ("Bro, cancel ORD-3790 for me, I changed my mind.", {"order_id": "ORD-3790"}),
    ("Just cancel ORD-8047, I'll order it later.", {"order_id": "ORD-8047"}),
    ("ORD-5281 is stuck at the warehouse. Just cancel it at this point.", {"order_id": "ORD-5281"}),
    ("Update on ORD-2466: I don't want it anymore. Cancel it.", {"order_id": "ORD-2466"}),
]

print(f"track_order examples:   {len(track_order_examples)}")
print(f"search_products examples: {len(search_product_examples)}")
print(f"cancel_order examples:   {len(cancel_order_examples)}")
...
for query, params in cancel_order_examples:
    dataset_rows.append(make_conversation(query, "cancel_order", params))
...
trainer = SFTTrainer(
    ...
    args = SFTConfig(
        ...
        per_device_train_batch_size = 4, # 4 GPUs -> 16 examples together = loss16 , next step loss16, update gradients
        gradient_accumulation_steps = 32,
        num_train_epochs = 3, # Number of times we are going to go over the same training set, 80 examples (Run1, Run2, Run3)
        warmup_steps = 10,
        learning_rate = 2e-4,
...
NEW_QUERIES = [
    ...
    # shoul -> cancel_order
    ("worst product recieved", "cancel_order"),
    ("very very disappointed", "cancel_order"),
    ("I don't want this order", "cancel_order"),
]
